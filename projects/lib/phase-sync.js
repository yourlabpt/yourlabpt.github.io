/**
 * Sincronização entre project.phases (plano #1, #2…) e requirement.phase.
 * Requisitos com «Fase 1», F1, etc. mapeiam para o nome canónico da fase #1 do plano.
 */
...
function syncRequirementsToPlanPhases(project, { nowIso = () => new Date().toISOString() } = {}) {
  const plan = ensureArray(project?.phases);
  ...
  for (const req of ensureArray(project?.requirements)) {
    ...
    if (!resolved) continue;
    const current = String(req?.phase || '').trim();
    if (normalizeForCompare(current) === normalizeForCompare(resolved)) continue;
    req.phase = resolved;
